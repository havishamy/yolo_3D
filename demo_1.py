...
class BBoxKalman:
    def __init__(self, x1, y1, x2, y2, max_jump=30):
        # 初始中心和宽高
  ...
  #  frame_objects = []
    ...
    for bbox, conf, cls in zip(bboxes, confs, clss):
        ...
        for tid, data in trackers.items():
            ...
            if iou_score > IOU_THRESH and conf>best_conf:
                best_conf = conf
                best_id = tid
    ...
    if len(trackers) > 0:
        main_tid = max(trackers.keys(), key=lambda tid: trackers[tid]["conf"])
    else:
        main_tid = None
    ...
    for tid, data in trackers.items():
        ...
        # 三角测量中心点
        if data.get("prev_center") is not None:
            ...
            trackers[tid]["trajectory"].append(pts3d.squeeze())
