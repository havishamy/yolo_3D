...
for i in range(total_objs - 1, -1, -1):
    box = boxes[i]
    cls = classes[i]
    conf = confidences[i]
    ...
    if results_sam[0].masks is None:
        ...
        continue
    mask = results_sam[0].masks.data[0].cpu().numpy()
    valid_mask = (mask==1).astype("uint8")
    mask_area = np.sum(valid_mask)
    print(f"目标 {i}（ID:{obj_id}）掩码大小：{mask_area}，开始裁剪...")
    mask = (mask * 255).astype("uint8")
        # 3. ✅ 检查掩码是否有大于0的有效区域
    if not np.any(mask > 0):
        print(f"目标 {i}（ID:{obj_id}）掩码为空（无有效区域），跳过")
        continue
    ...
    all_obj_info.append(id_to_info[obj_id])

    print(f"目标 {i} - 分配 ID:{obj_id}, 类别:{int(cls)}, 置信度:{conf:.2f}")
