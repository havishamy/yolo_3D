        ...
        while True:
            ...
            try:
                ...
                for i in range(total_objs):
                    ...
                    x2, y2 = min(640-1, x2), min(480-1, y2)
                    if x1 >= x2 or y1 >= y2:
            ...
            if isinstance(results_sam, list) and len(results) > 0:
                res = results_sam[0]
            else:
            ...
            try:
                ...
                has_id= res.boxes.cls.cpu().numpy()
                mask=res.masks.data.cpu().numpy()
                for b, tid,mask_ in zip(boxes_xyxy, has_id,mask):
            ...
            for bbox, tid ,mask in zip(boxes, ids,masks):
